...
def update_existing_asset(
    *,
    ...
    if asset_in.symbol and asset_in.symbol.upper() != db_asset.symbol.upper():
        existing_asset_with_new_symbol = crud.get_asset_by_symbol(
            db, symbol=asset_in.symbol
        )
        if (
            existing_asset_with_new_symbol
            and existing_asset_with_new_symbol.id != asset_id
        ):
            raise HTTPException(
                status_code=400, detail="Another asset with this symbol already exists."
            )
