...
@patch(
    "app.services.data_providers.alpha_vantage_provider.fetch_av_stock_current_price"
)
...
def test_get_current_price_stock_cache_miss_then_hit(
    ...
    print(
        f"DEBUG Orchestrator Test: Stock Price - Calling get_current_price for {symbol} (2nd time)"
    )
...
@patch("app.services.financial_data_orchestrator.shared_cache.get_shared_cache")
@patch(
    "app.services.data_providers.alpha_vantage_provider.fetch_av_crypto_current_price"
)
@patch("app.services.data_providers.yahoo_finance_provider.fetch_yf_current_price")
def test_get_current_price_crypto_cache_miss_then_hit(
    ...
    print(
        f"DEBUG Orchestrator Test: Calling get_current_price for {symbol} (2nd time, expect shared_cache hit)"
    )
    price2 = orchestrator.get_current_price(symbol, asset_type)
...
def test_get_current_price_provider_returns_none(
    ...
    assert (
        cache_key not in orchestrator._cache["price_cache"]
    ), "None should not be cached for price"
...
def test_get_current_price_cache_expiration(
    ...
    print(
        f"DEBUG Orchestrator Test: Cache Expiration - Calling get_current_price for {symbol} (2nd time after expiry)"
    )
    price2 = orchestrator.get_current_price(symbol, asset_type)
    ...
    monkeypatch.setattr(
        "app.cache.shared_cache.CACHE_DURATION_SECONDS", original_cache_duration
    )
...
@patch("app.services.financial_data_orchestrator.shared_cache.get_shared_cache")
@patch(
    "app.services.data_providers.alpha_vantage_provider.fetch_av_stock_historical_data"
)
...
def test_get_historical_data_stock_cache_miss_then_hit(
    ...
    cache_key = (
        f"history:{symbol.upper()}_{asset_type or 'unknown'}_{yf_period_expected}"
    )

    mock_hist_data_from_yf = [
        {
            "date": date(2023, 1, 1),
            "close": 250.0,
            "open": 248.0,
    ...
    mock_get_shared_cache.assert_called_once_with(cache_key)
    mock_fetch_yf_stock_hist.assert_called_once_with(
        symbol, asset_type, period=yf_period_expected
    )
    ...
    mock_get_shared_cache.return_value = [
        {
            "date": "2023-01-01",
            "close": 250.0,
            "open": 248.0,
    ...
    print(
        f"DEBUG Orchestrator Test: Hist Stock - Calling get_historical_data for {symbol} (2nd time)"
    )
...
@patch("app.services.financial_data_orchestrator.shared_cache.get_shared_cache")
@patch(
    "app.services.data_providers.alpha_vantage_provider.fetch_av_crypto_historical_data"
)
@patch("app.services.data_providers.yahoo_finance_provider.fetch_yf_historical_data")
def test_get_historical_data_crypto_cache_miss_then_hit(
    ...
    mock_fetch_av_crypto_hist.return_value = [
        {"date": date(2022, 1, 5), "close": 99999.0}
    ]
    ...
    assert hist1 == mock_hist_data_from_yf
    mock_get_shared_cache.assert_called_once_with(cache_key)
    mock_fetch_yf_crypto_hist.assert_called_once_with(
        symbol, asset_type, period=yf_period_expected
    )
    ...
    print(
        f"DEBUG Orchestrator Test: Hist Crypto - Calling get_historical_data for {symbol} (2nd time)"
    )
    hist2 = orchestrator.get_historical_data(symbol, asset_type, outputsize)
...
def test_get_historical_data_provider_returns_none(
    ...
    mock_fetch_av_stock_hist.assert_called_once_with(
        symbol.upper(), outputsize=outputsize
    )
...
def test_get_historical_data_provider_returns_empty_list(
    ...
    assert (
        history == []
    ), "History should be an empty list if AV provider returns an empty list"
