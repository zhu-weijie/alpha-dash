...
def mock_yf_ticker():
    ...
    with patch("app.services.data_providers.yahoo_finance_provider.yf.Ticker", return_value=mock_ticker_instance) as mock_ticker_class:
...
def test_map_symbol_for_yfinance_crypto():
    ...
    assert yf_provider._map_symbol_for_yfinance("XYZ", "crypto") == "XYZ" # Unknown crypto, returns original
...
def test_fetch_yf_current_price_from_history(mock_yf_ticker):
    ...
    mock_history_df = pd.DataFrame({
        'Open': [150.0], 'High': [152.0], 'Low': [149.0], 'Close': [151.25], 'Volume': [100000]
...
def test_fetch_yf_current_price_from_info(mock_yf_ticker):
    mock_ticker_instance, _ = mock_yf_ticker
    mock_ticker_instance.history.return_value = pd.DataFrame() # Simulate history failing
...
def test_fetch_yf_historical_data_success(mock_yf_ticker):
    mock_ticker_instance, _ = mock_yf_ticker
    mock_history_df = pd.DataFrame({
        'Open': [150.0, 151.0], 'High': [152.0, 153.0], 'Low': [149.0, 150.0],
        'Close': [151.25, 152.50], 'Volume': [100000, 120000]
    }, index=[pd.Timestamp('2023-10-26'), pd.Timestamp('2023-10-27')])
    mock_ticker_instance.history.return_value = mock_history_df

    history = yf_provider.fetch_yf_historical_data("AAPL", asset_type="stock", period="1mo")
...
def test_fetch_yf_historical_data_empty(mock_yf_ticker, capsys):
    ...
    assert history is None # Or [] depending on how your provider handles truly empty DF
    captured = capsys.readouterr()
