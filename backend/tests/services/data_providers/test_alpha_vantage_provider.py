...
MOCK_AV_STOCK_TIME_SERIES_DAILY_SUCCESS = {
    ...
    "Time Series (Daily)": {
        "2023-10-27": {"1. open": "136.50", "2. high": "137.40", "3. low": "135.66", "4. close": "136.99", "5. volume": "2856020"},
        "2023-10-26": {"1. open": "134.90", "2. high": "136.00", "3. low": "134.50", "4. close": "135.82", "5. volume": "3500000"}
    }
}
MOCK_AV_CRYPTO_DIGITAL_CURRENCY_DAILY_SUCCESS = {
    "Meta Data": {"2. Digital Currency Code": "ETH", "4. Market Code": "USD"},
    "Time Series (Digital Currency Daily)": {
        "2023-10-27": {"1a. open (USD)": "1800.00", "2a. high (USD)": "1850.00", "3a. low (USD)": "1790.00", "4a. close (USD)": "1830.00", "5. volume": "12345.678"},
        "2023-10-26": {"1a. open (USD)": "1750.00", "2a. high (USD)": "1780.00", "3a. low (USD)": "1700.00", "4a. close (USD)": "1760.00", "5. volume": "23456.789"}
    }
}
MOCK_AV_API_NOTE_RESPONSE = {
    "Note": "Thank you for using Alpha Vantage! Our standard API call frequency is..."
}
MOCK_AV_API_INFORMATION_RESPONSE = { # Sometimes "Information" is used for errors/limits
...
def test_fetch_av_stock_current_price_parsing_error(requests_mock, capsys):
    symbol = "BADPARSE"
    url = f"{MOCK_AV_BASE_URL}?apikey={settings.ALPHA_VANTAGE_API_KEY}&function=GLOBAL_QUOTE&symbol={symbol.upper()}"
    requests_mock.get(url, json={"Global Quote": {"wrong_key": "123"}}) # Missing "05. price"
...
def test_fetch_av_crypto_current_price_success(requests_mock):
    ...
    price = av_provider.fetch_av_crypto_current_price(crypto_symbol, market_currency=market)
    assert price == expected_price

def test_fetch_av_crypto_current_price_api_information(requests_mock, capsys): # Test with "Information" key
    crypto_symbol = "ETH"
    market = "USD"
    url = f"{MOCK_AV_BASE_URL}?apikey={settings.ALPHA_VANTAGE_API_KEY}&function=CURRENCY_EXCHANGE_RATE&from_currency={crypto_symbol.upper()}&to_currency={market.upper()}"
...
def test_fetch_av_stock_historical_data_empty_series(requests_mock, capsys):
    ...
    outputsize = "compact"
    url = f"{MOCK_AV_BASE_URL}?apikey={settings.ALPHA_VANTAGE_API_KEY}&function=TIME_SERIES_DAILY&symbol={symbol.upper()}&outputsize={outputsize}"
    requests_mock.get(url, json={"Meta Data": {}, "Time Series (Daily)": {}}) # Empty time series
    
    history = av_provider.fetch_av_stock_historical_data(symbol, outputsize=outputsize)
    assert history is None # Or [] depending on provider logic for truly empty series after parsing
    captured = capsys.readouterr()
...
def test_fetch_av_crypto_historical_data_success(requests_mock):
    crypto_symbol = "ETH"
    market = "USD"
    outputsize = "compact" # Note: Provider might ignore this for AV crypto and truncate later
    ...
    history = av_provider.fetch_av_crypto_historical_data(crypto_symbol, market_currency=market, outputsize=outputsize)
    assert history is not None
    assert len(history) == 2
...
def test_av_provider_no_api_key(monkeypatch, capsys):
    ...
    captured = capsys.readouterr()
    assert captured.out.count("AV_PROVIDER Warning: ALPHA_VANTAGE_API_KEY not configured.") >= 4 # Check for multiple calls
...
def test_av_provider_network_error(requests_mock, capsys):
    ...
    url = f"{MOCK_AV_BASE_URL}?apikey={settings.ALPHA_VANTAGE_API_KEY}&function=GLOBAL_QUOTE&symbol={symbol.upper()}"
    requests_mock.get(url, exc=requests.exceptions.ConnectionError("Simulated network failure"))
